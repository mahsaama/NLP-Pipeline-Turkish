...
class Stemmer:
    ...
    def __init__(self):
        self.reserved_stems_list = utils.load_words("./DATA/reserved_word_list.txt")
        self.small_stems_list = utils.load_words("./DATA/small_words_list.txt")
    ...
    def stemmer_helper(self, token, recursion_count, suffix_list, avg_length):
        ...
        if token not in self.reserved_stems_list and len(token) > 3:
                ...
                if delete:
                    for type in types:
                        if token[-length:] == type:
                            ...
                            if (
                                token not in self.reserved_stems_list
            ...
            if (
                len(token) <= avg_length or recursion_count > 3
            ):  # lenghti azaltıp bazı korunan wordler listesi olmalı
                return token
            else:
                recursion_count += 1
                # Check bigger stems from a list and return them directly:
                # 'Ayakkabı' gibi
                return Stemmer.stemmer_helper(
                    self, token, recursion_count, suffix_list, avg_length
                )
    ...
    def stemmer(self, token):
        ...
        noun_stem = Stemmer.stemmer_helper(
            self, noun_stem, 1, NounSuffixes.noun_suffix_list, 7
        )
        # Also, Check if the stem of this noun is actually a verb
        noun_stem = Stemmer.stemmer_helper(
            self, noun_stem, 1, VerbSuffixes.verb_to_noun_suffix_list, 5
        )

        verb_stem = Stemmer.stemmer_helper(
            self, verb_stem, 1, VerbSuffixes.verb_suffix_list, 5
        )
        nominal_verb_stem = Stemmer.stemmer_helper(
            self, nominal_verb_stem, 1, VerbSuffixes.nominal_verb_suffix_list, 7
        )
        derivational_noun_stem = Stemmer.stemmer_helper(
            self,
            derivational_noun_stem,
            1,
            NounSuffixes.noun_derivational_suffix_list,
            6,
        )
        ...
        if len1 < len2:
            ...
            stem = Stemmer.stemmer_helper(
                self, stem, 1, VerbSuffixes.verb_to_noun_suffix_list, 5
            )
        ...
        if len3 < len(stem):
            ...
            stem = Stemmer.stemmer_helper(
                self, stem, 1, VerbSuffixes.verb_suffix_list, 5
            )
